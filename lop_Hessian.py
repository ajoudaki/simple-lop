...
import torch.func as func # Added for Hessian tools
...
class Muon(Optimizer):
    ...
    def __init__(self, params, lr=1e-3,
        ...
        # Muon-specific Newton-Schulz coefficients from paper
        self.a = 3.4445
        self.b = -4.7750
        self.c = 2.0315
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if p.dim() == 2:
                    ...
                    # We implement this with DECOUPLED weight decay, as inspired by AdamW
...
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# START: Hessian Analysis Tools (Lanczos, KPM)
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

def _pytree_utils(pytree):
    """
    Manually flattens a 'pytree' (a dict of tensors) into a 
    ...
    # We must iterate in a consistent order
    for key, tensor in pytree.items():
        original_keys.append(key)
        original_shapes.append(tensor.shape)
        num_elements_per_tensor.append(tensor.numel())

    # Handle empty pytree (like empty buffers)
    if not original_keys:
        flat_tensor = torch.empty(0, device=next(pytree.values()).device if pytree else 'cpu')
    else:
        flat_tensor = torch.cat([
            pytree[key].reshape(-1) for key in original_keys
        ])
...
class LanczosHessian:
    """
    Encapsulates the stable Lanczos algorithm to find Hessian eigenvalues.
    """
    ...
    def _get_hvp_fn(self, x_batch, y_batch):
        """
        Returns a "ready-to-use" function that computes H*v for the model
        and data provided, operating on flat vectors.
        """
        
        def stateless_loss_fn(params, buffers, x, y):
            # functional_call(model.forward) returns (p, logits)
            # We must select 'logits' and squeeze BOTH logits and y
            _, logits = func.functional_call(self.model, (params, buffers), (x,))
            return self.loss_fn(logits.squeeze(), y.squeeze())

        flat_params, unflatten_fn = _pytree_utils(self.params_dict)

        def hvp_fn_flat(v_flat):
            v_pytree = unflatten_fn(v_flat)
            primals_in = (self.params_dict, self.buffers_dict, x_batch, y_batch)
            tangents = (
                v_pytree,                   # Tangent for params
                {},                         # Tangent for buffers (matches structure)
                torch.zeros_like(x_batch),  # Zero tangent for x_batch
                torch.zeros_like(y_batch)   # Zero tangent for y_batch
            )
            
            grad_fn = func.grad(stateless_loss_fn, argnums=0)
            _, hvp_pytree = func.jvp(grad_fn, primals_in, tangents)
            
            hvp_flat, _ = _pytree_utils(hvp_pytree)
            return hvp_flat

        return hvp_fn_flat, flat_params.numel()

    def run(self, x_batch, y_batch, k):
        """
        Runs the stable Lanczos algorithm for k iterations.
        """
        
        hvp_fn, dim = self._get_hvp_fn(x_batch, y_batch)
        
        print(f"    [Hessian] Running Stable Lanczos for k={k} iterations...")
        ...
        # --- FIX ---
        # Initialize 'v' as Double to match the model and HVP output
        v = torch.randn(dim, device=self.device).double()
        # --- END FIX ---
        ...
        for j in range(k):
            q_j = Q[j]
            w = hvp_fn(q_j)
            alpha = torch.dot(w, q_j)
            alphas.append(alpha.item())

            if j == 0:
                w = w - alpha * q_j
            else:
                w = w - alpha * q_j - betas[j-1] * Q[j-1]
        ...
        T.diagonal(offset=0).copy_(torch.tensor(alphas, device=self.device))
        if betas: 
            T.diagonal(offset=-1).copy_(torch.tensor(betas, device=self.device))
            T.diagonal(offset=1).copy_(torch.tensor(betas, device=self.device))
...
class KPMHessian:
    """
    Estimates the Hessian eigenvalue density using the Kernel Polynomial Method.
    """
    def __init__(self, model, loss_fn, device='cpu'):
        self.model = model
        self.loss_fn = loss_fn
    ...
    def _get_hvp_fn(self, x_batch, y_batch):
        ...
        Returns a "ready-to-use" function that computes H*v for the model
        and data provided, operating on flat vectors.
        (This is duplicated from LanczosHessian for modularity)
        """
    ...
    def _get_rescaled_hvp_fn(self, x_batch, y_batch, a, b):
        """
        Returns an HVP function for the *rescaled* Hessian H',
        where H' eigenvalues are in [-1, 1].
        """
        hvp_fn_flat, dim = self._get_hvp_fn(x_batch, y_batch)
        ...
        # H'v = (1/d) * (Hv - cv)
        def rescaled_hvp_fn(v):
            return (1.0 / d) * (hvp_fn_flat(v) - c * v)
            
        return rescaled_hvp_fn, dim

    def run(self, x_batch, y_batch, M, R, a, b):
        """
        Computes the KPM moments.
        """
        hvp_prime_fn, dim = self._get_rescaled_hvp_fn(x_batch, y_batch, a, b)
        
        print(f"    [Hessian] Running KPM for M={M} moments, R={R} vectors...")
        
        moments_sum = torch.zeros(M, device=self.device, dtype=torch.double) # Use double for sum stability

        for _ in range(R):
            # Use random vectors v with entries {+1, -1}
            v = (torch.randint(0, 2, (dim,), device=self.device) * 2.0 - 1.0).double()
            
            # Chebyshev recurrence
            v_0 = v
            v_1 = hvp_prime_fn(v_0)
            
            moments_sum[0] += torch.dot(v, v_0)
            moments_sum[1] += torch.dot(v, v_1)
            
            for m in range(1, M - 1):
                v_next = 2 * hvp_prime_fn(v_1) - v_0
                moments_sum[m+1] += torch.dot(v, v_next)
                
                v_0, v_1 = v_1, v_next # Update
        
        # Normalize to get the final moments
        moments = moments_sum / (dim * R)
        return moments

    @staticmethod
    def _jackson_kernel(M, m):
        """Calculates Jackson kernel coefficients."""
        M_p1 = M + 1
        m_pi = m * math.pi / M_p1
        
        tan_val = math.tan(math.pi / M_p1) 
        return ((M_p1 - m) * torch.cos(m_pi) + 
                torch.sin(m_pi) / tan_val) / M_p1

    @staticmethod
    def reconstruct_density(moments, x_grid, a, b):
        """
        Reconstructs the density from moments on an energy grid 'x_grid'.
        'x_grid' should be in the *original* energy scale [a, b].
        """
        M = len(moments)
        device = moments.device

        # 1. Rescale x_grid to lambda_grid in [-1, 1]
        c = (b + a) / 2.0
        d = (b - a) / 2.0
        
        # --- FIX 1 ---
        # Ensure x_grid is Double to match moments
        lambda_ = ((x_grid.double() - c) / d) 
        # --- END FIX ---
        
        lambda_ = torch.clamp(lambda_, -1.0 + 1e-9, 1.0 - 1e-9)

        # 2. Get Jackson kernel damping coefficients
        # --- FIX 2 ---
        # Ensure m is Double to match moments
        m = torch.arange(M, device=device).double()
        # --- END FIX ---
        
        g = KPMHessian._jackson_kernel(M-1, m)
        damped_moments = g * moments
        
        # 3. Compute density
        acos_lambda = torch.acos(lambda_)
        T_m_x = torch.cos(m.unsqueeze(1) * acos_lambda.unsqueeze(0))
        
        damped_moments[0] *= 0.5 # Handle m=0 term separately
        density = 2 * torch.einsum('m,mx->x', damped_moments, T_m_x)
        
        # 4. Apply prefactors
        prefactor = 1.0 / (math.pi * torch.sqrt(1.0 - lambda_**2))
        rescaled_density = (prefactor * density) / d
        
        return rescaled_density

# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# END: Hessian Analysis Tools
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
def effective_rank(Z, eps=1e-10):
    ...
    C = C + eps * torch.eye(C.shape[0], device=C.device, dtype=C.dtype)  # Regularize
...
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# START: NEW Hessian Analysis Helper Function
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

def run_hessian_analysis(model, loss_fn, x_batch, y_batch, args, save_name):
    """
...
def run(args):
    ...
    # Define it once to pass to helper functions
    ...
    if args.compute_hessian:
        os.makedirs("out/hessian_plots", exist_ok=True)
    ...
    for t in range(total_tasks):
        ...
        # --- (NEW) HESSIAN: Compute at task START ---
        if args.compute_hessian:
            run_hessian_analysis(net, criterion, X, y, args, f"task_{t+1}_start")
        ...
        for s in range(steps):
            ...
            # Use the pre-defined criterion
            loss = criterion(logits.squeeze(), y.squeeze())
            ce = loss.item()
        ...
        # --- (NEW) HESSIAN: Compute at task END ---
        if args.compute_hessian:
            run_hessian_analysis(net, criterion, X, y, args, f"task_{t+1}_end")
    ...
    if args.fresh_baseline > 0:
        ...
        for t in range(args.fresh_baseline):
            ...
            for s in range(fresh_steps):
                ...
                f_loss = criterion(f_logits.squeeze(), y.squeeze())
                f_loss.backward()
                f_opt.step()
                
            _, final_logits = f_net(X, capture=False)
            final_ce_fresh = criterion(final_logits.squeeze(), y.squeeze()).item()
...
if __name__ == "__main__":
    ...
    # --- (NEW) Hessian Analysis Arguments ---
    ap.add_argument("--compute_hessian", action="store_true",
                    help="Compute Hessian eigenvalue density at task switches")
    ap.add_argument("--lanczos_k", type=int, default=50,
                    help="Lanczos iterations for spectral bounds")
    ap.add_argument("--kpm_moments", type=int, default=200,
                    help="KPM moments (resolution)")
    ap.add_argument("--kpm_vectors", type=int, default=10,
                    help="KPM random vectors (accuracy)")
